...
def validate_model(sub, eeg_model, dataloader, device, config):
    eeg_model.eval()
    total_loss = 0
    correct = 0
    total = 0
...
def train_model(sub, eeg_model, dataloader, optimizer, device, config):
    eeg_model.train()
    total_loss = 0
    correct = 0
    total = 0
...
def main_train_loop(sub, current_time, eeg_model, train_dataloader, val_dataloader, test_dataloader, optimizer, device,
                    text_features_test_all, img_features_test_all, config, logger=None):
    ...
    for epoch in range(config.epochs):
        # Train the model
        train_loss, train_accuracy, features_tensor = train_model(sub, eeg_model, train_dataloader, optimizer, device, config=config)
        if (epoch +1) % 5 == 0:                    
        ...
        # validate the model
        val_loss, val_accuracy = validate_model(sub, eeg_model, val_dataloader, device, config=config)
        ...
        print(f"Epoch {epoch + 1}/{config.epochs} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Test Loss: {test_loss:.4f}, Test Acc: {test_accuracy:.4f},")
        print(f"Epoch {epoch + 1}/{config.epochs} - Top5 Acc: {top5_acc:.4f}, v2 Acc:{v2_acc} - v4 Acc:{v4_acc} - v10 Acc:{v10_acc} - v50 Acc:{v50_acc} - v100 Acc:{v100_acc}")
    ...
    logger.finish()
    return results, best_epoch_info
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default="/proj/rep-learning-robotics/users/x_nonra/eeg_asif_img/data/things_eeg_2/Preprocessed_data_250Hz", help='Path to the EEG dataset')
    ...
    parser.add_argument('--img_model_type', type=str, default='ViT-B-32', help='Image encoder model type')
    parser.add_argument('--precompute_model_name', type=str, default='dreamsim_clip_vitb32', help='Precomputed image embeddings model type')
    parser.add_argument('--precompute_img', action='store_true', help='Use precomputed image embeddings')
    parser.add_argument('--average_repetitions', action='store_true', help='Average repetitions')
    ...
    for sub in subjects:
        ...
        if args.insubject:
            train_dataset = EEGDataset(args.data_path, subjects=[sub], train=True, model_type=args.img_model_type, 
                                       precompute_img=args.precompute_img, precompute_model_name=args.precompute_model_name, average_repetitions=args.average_repetitions)
            test_dataset = EEGDataset(args.data_path, subjects=[sub], train=False, model_type=args.img_model_type, 
                                       precompute_img=args.precompute_img, precompute_model_name=args.precompute_model_name, average_repetitions=args.average_repetitions)
        else:
            train_dataset = EEGDataset(args.data_path, exclude_subject=sub, subjects=subjects, train=True, model_type=args.img_model_type, 
                                       precompute_img=args.precompute_img, precompute_model_name=args.precompute_model_name, average_repetitions=args.average_repetitions)
            test_dataset = EEGDataset(args.data_path, exclude_subject=sub, subjects=subjects, train=False, model_type=args.img_model_type, 
                                       precompute_img=args.precompute_img, precompute_model_name=args.precompute_model_name, average_repetitions=args.average_repetitions)

        train_dataset, val_dataset = torch.utils.data.random_split(train_dataset, [0.9, 0.1], generator=torch.Generator().manual_seed(args.seed))
        ...
        results, best_results = main_train_loop(sub, current_time, eeg_model, train_loader, val_loader, test_loader, optimizer, device, 
                                text_features_test_all, img_features_test_all, config=args, logger=args.logger)


        # Save results to a CSV file
        results_dir = os.path.join(args.output_dir, args.precompute_model_name, sub)
        ...
        with open(best_results_file, 'wb') as file:
            print("best epoch info = ", best_results)
